...
class PaypalController extends Controller
{
    ...
    public function subscriptions(){
        ...
        {
            $trial_version_msg = __("Your trial version has finished, check the subscription plans here.");
        } else if ($user->trial_end >= date('Y-m-d') && $user->is_subscribed != 1)
        {
            $remain = (strtotime($user->trial_end) - strtotime(date('Y-m-d')))/60/60/24;
            $trial_version_msg = __("Your trial version will be expired after")." " . $remain ." ". __("days").".";
        } else {
            $trial_version_msg = __("Your trial version is started now. it will be expire after 21 days.");
